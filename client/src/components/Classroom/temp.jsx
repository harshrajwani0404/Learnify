...
const ClassSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Success message and state reset
...
// Input fields configuration with validation messages
const fields = [
  {
    id: "title",
    label: "Title",
  ...
  return (
    <div className="container mx-auto px-4 py-12 bg-gray-100 min-h-screen">
          ...
          >
            <Link to={`/home/${subject}/${item.name.toLowerCase()}`} className="block w-full h-full">
              <motion.div 
                ...
                whileTap={{ scale: 0.95 }}
              >
                <item.icon className="w-12 h-12 mx-auto mb-4 text-blue-500 group-hover:text-blue-600" />
                <h2 className="text-2xl font-bold mb-2 group-hover:text-blue-600">{item.name}</h2>
      ...
      {user && user.type === "teacher" && (
        <motion.button
          className="fixed bottom-8 right-8 bg-blue-600 text-white text-lg font-semibold py-3 px-6 rounded-full shadow-lg transition-all duration-300 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 flex items-center"
